...
const serial = {
  ...
  actions: {
    ...
    storeSingleCLogic(context, body) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.init();
        ApiService.post(
          `/api/v1/project/${projectId}/serial/config/${configId}/clogic/new`,
          {
            rule,
            data,
            next_success,
          ...
          (response) => {
            context.commit("addSingleCLogicSerial", {
              configId,
              cLogic: response.data,
            });
            resolve(response);
          },
          (error) => {
    ...
    deleteCLogic(context, data) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.init();
        ApiService.delete(
          `/api/v1/project/${projectId}/serial/config/${configId}/clogic/${cLogicId}`
        ).then(
          (response) => {
            context.commit("removeSpecificCLogic", {
              projectId,
              configId,
              cLogicId,
            });
    ...
    deleteSpecificConfigFile(context, data) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.init();
        ApiService.delete(
          `/api/v1/project/${projectId}/serial/configure-file/${configFileId}`
        ).then(
          (response) => {
            context.commit("removeSpecificConfigFile", {
              projectId,
              configFileId,
            });
            resolve(response);
    ...
    updateSingleCLogic(context, body) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.init();
        ApiService.put(
          `/api/v1/project/${projectId}/serial/config/${configId}/clogic`,
          {
            id,
            rule,
            data,
          ...
          (response) => {
            context.commit("updateSingleCLogicSerial", {
              configId,
              id,
              cLogic: response.data,
            });
  ...
  mutations: {
    ...
    addSingleCLogicSerial(state, data) {
      ...
      let configIndex = state.serial.configures.findIndex(
        (e) => e.id === configId
      );
      if (configIndex >= 0) {
    ...
    removeSpecificCLogic(state, data) {
      ...
      if (configIndex >= 0) {
        state.serial.configures[configIndex].c_logics = state.serial.configures[
          configIndex
        ].c_logics.filter((c) => c.id !== cLogicId);
      }
    },
    removeSpecificConfigFile(state, data) {
      const { configFileId } = data;
      state.serial.configures = state.serial.configures.filter(
        (c) => c.id !== configFileId
      );
    },
    ...
    updateSingleCLogicSerial(state, data) {
      ...
      if (configIndex >= 0) {
        let cLogicIndex = state.serial.configures[
          configIndex
        ].c_logics.findIndex((c) => c.id === id);
        if (cLogicIndex >= 0) {
    ...
    updateSingleCLogic(state, data) {
      const { configId, value } = data;
      let confIndex = state.serial.configures.findIndex(
        (e) => e.id === configId
      );
      if (confIndex >= 0) {
        let cLogicIndex = state.serial.configures[confIndex].c_logics.findIndex(
          (e) => e.id === value.id
        );
        if (cLogicIndex >= 0) {
          let temp = [...state.serial.configures];

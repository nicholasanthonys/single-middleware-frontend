...
const parallel = {
  ...
  actions: {
    ...
    deleteSpecificCLogic(context, data) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.init();
        ApiService.delete(
          `/api/v1/project/${projectId}/parallel/clogic/${cLogicId}`
        ).then(
          (response) => {
    ...
    deleteSpecificConfigFile(context, data) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.init();
        ApiService.delete(
          `/api/v1/project/${projectId}/parallel/configure-file/${configFileId}`
        ).then(
          (response) => {
    ...
    updateSingleConfigParallel(context, data) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.put(`/api/v1/project/${projectId}/parallel/config`, {
          ...
          (response) => {
            context.commit("updateSingleConfigParallel", {
              id,
              configure_id,
              alias,
              loop,
    ...
    storeFailureResponse(context, data) {
      return new Promise((resolve, reject) => {
        const { transform, status_code, adds, modifies, deletes, projectId } =
          data;
        ApiService.init();
        ApiService.post(
          `/api/v1/project/${projectId}/parallel/failure-response`,
          {
            transform,
            status_code,
            adds,
    ...
    updateSingleCLogicParallel(context, body) {
      return new Promise((resolve, reject) => {
        ...
        ApiService.put(`/api/v1/project/${projectId}/parallel/clogic`, {
          ...
          (response) => {
            context.commit("updateSingleCLogicParallel", {
              id,
  ...
  mutations: {
    ...
    removeSpecificCLogic(state, data) {
      ...
      state.parallel.c_logics = state.parallel.c_logics.filter(
        (c) => c.id !== cLogicId
      );
    },
    removeSpecificConfigFile(state, data) {
      const { configFileId } = data;
      state.parallel.configures = state.parallel.configures.filter(
        (c) => c.id !== configFileId
      );
    },

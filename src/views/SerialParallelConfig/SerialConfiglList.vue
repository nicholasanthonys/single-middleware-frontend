<template>
  <div>
        ...
        <div class="column" style="height: 200px">
          ...
          <div class="col">
            <div
              ...
              style="height: 200px; background: #f3f3f3"
            >
    ...
    <q-dialog
      v-model="dialogDeleteConfig"
      persistent
      v-if="selectedConfigureSerial"
    >
      <q-card>
        <q-card-section class="row items-center">
          <q-avatar icon="delete" color="negative" text-color="white" />
          <span class="q-ml-sm"
            >Are you sure want to delete config serial with id
            {{ selectedConfigureSerial.id }} ?
          </span>
        </q-card-section>
...
export default {
  ...
  methods: {
    ...
    onConfirmConfigSerial() {
      this.selectedConfigureSerial =
        this.serial.configures[this.serial.configures.length - 1];
      //this.dialog = false;
    },
    selectConfigureSerial(index) {
      this.selectedConfigureSerial = this.serial.configures[index];

      (this.mode = "edit"), (this.dialog = true);
    },
